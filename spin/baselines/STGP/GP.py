...
class GP:
    ...
    def __init__(self, df, target_var, target_name, cov_func_name):
        ...
        self.sigma_n = np.sqrt(0.1 * np.var(self.y_std))  # Observation noise root variance, σ_n in Eq.4
        self.K = self.cov_func(self.X_std[:1000, :], self.X_std[:1000, :])  # Data covariance
    ...
    # Method to find squared exponential covariance given covariate matrices and hyperparameters
    ...
    def log_marginal_likelihood(self):
        ...
        slog = np.linalg.slogdet(self.K + np.eye(self.K.shape[0]) * self.sigma_n ** 2)
        lml2 = -.5 * slog[0] * slog[1]
    ...
    def fit(self, x0=None, bounds=None, verbose=False):
        ...
        def eval_params(params):
            ...
            self.set_hyper_params(params[:-1], params[len(params) - 1])
        ...
        res = differential_evolution(eval_params, bounds)
    ...
    def init_hps(self):
        raise NotImplementedError('Implemented this method in subclass.')
