...
class STGP(SGP):
    # Constructor method
    def __init__(self, df, target_var, target_name, long_low, long_up, lat_low, lat_up,
                 alt_low, alt_up, date_low, cov_func_name):
        self.date_low = date_low
        SGP.__init__(self, df, target_var, target_name, long_low, long_up, lat_low, lat_up,
                     alt_low, alt_up, None, cov_func_name)
        self.X_std, self.y_std = self.sample()
        self.min_date = int(np.min(self.X[:, 3]))
        self.max_date = int(np.max(self.X[:, 3]))
        self.dates = np.arange(self.min_date, self.max_date + 1)
        self.n_dates = len(self.dates)

    def sample(self, size=2000):
        index = np.random.randint(0, len(self.X_std), size=size)
        X = self.X_std[index, :]
        y = self.y_std[index, :]
        return X, y

    def sampleX(self, size=1000):
        index = np.random.randint(0, len(self.X_std), size=size)
        X = self.X[index, :]
        y = self.y[index, :]
        return X, y
    ...
    def filter(self, df, target_var):
        np_filter = (df['long'] >= self.long_low) & (df['long'] <= self.long_up) & \
                    (df['lat'] >= self.lat_low) & (df['lat'] <= self.lat_up) & \
                    (df['alt'] >= self.alt_low) & (df['alt'] <= self.alt_up) & \
    ...
    def select(self, df):
        ...
        return df_filt[['long', 'lat', 'alt', 'date']].values, \
    ...
    def init_hps(self):
        ...
        # Function standard deviation σ_k in eq.4
        ...
        if self.cov_func_name == "Squared exponential":
            hps = hps + [np.prod(np.ptp(self.X_std[:, 0:3], axis=0)) / np.sqrt(len(self.y))]
        ...
        hps = hps + [np.ptp(self.X_std[:, 3]) / np.sqrt(len(self.y))]
    ...
    def cov_func(self, x_1, x_2):
        ...
        if self.cov_func_name == "Squared exponential":
            K_s = self.se_cov_func(x_1[:, 0:3], x_2[:, 0:3], self.hyper_params[0:2])
