...
class SGP(GP):
    ...
    def __init__(self, df, target_var, target_name, long_low, long_up, lat_low, lat_up,
                 alt_low, alt_up, date, cov_func_name):
        self.long_low, self.long_up = long_low, long_up
        self.lat_low, self.lat_up = lat_low, lat_up
        self.alt_low, self.alt_up = alt_low, alt_up
        self.date = date
        self.np_filter = self.filter(df, target_var)
        self.sites = df['sensor'].values[self.np_filter, np.newaxis]
    ...
    def filter(self, df, target_var):
        np_filter = (df['long'] >= self.long_low) & (df['long'] <= self.long_up) & \
                    ...
                    (df['alt'] >= self.lat_low) & (df['alt'] <= self.lat_up) & \
