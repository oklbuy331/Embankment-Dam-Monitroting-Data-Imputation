...
from spin.baselines import BRITS, BiGAN
from spin.datasets import PoreWaterPressure
from spin.imputers import SPINImputer, BRITSImputer
...
def load_model(exp_dir, exp_config, dm):
    ...
    imputer = imputer_class(model_class=model_cls,
...
def run_experiment(args):
    ...
    for seed in seeds:
        ...
        train_output = trainer.predict(imputer, dataloaders=dm.train_dataloader(shuffle=False))
        val_output = trainer.predict(imputer, dataloaders=dm.val_dataloader(shuffle=False))
        test_output = trainer.predict(imputer, dataloaders=dm.test_dataloader(shuffle=False))

        y_hat = torch.cat([train_output['y_hat'], val_output['y_hat'], test_output['y_hat']], dim=0)[:, 35, :].squeeze(-1)
        y_hat = casting.numpy(y_hat)

        y_true = torch.cat([train_output['y'], val_output['y'], test_output['y']], dim=0)[:, 35, :].squeeze(-1)
        y_true = casting.numpy(y_true)

        mask = torch.cat([train_output['mask'], val_output['mask'], test_output['mask']], dim=0)[:, 35, :].squeeze(-1)
        ...
        print(f'SEED {seed} - Test MAE: {check_mae:.2f} MSE: {check_mse:.2f} MRE: {check_mre:.2f}')
